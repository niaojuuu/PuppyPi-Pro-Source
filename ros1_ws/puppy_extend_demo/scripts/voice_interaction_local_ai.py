...
"""
本地AI语音交互 - 全离线方案
架构: WonderEchoPro唤醒 → 麦克风录音 → sherpa-onnx ASR → Ollama LLM → 动作执行 + sherpa-onnx TTS
运行环境: 树莓派5 (16GB)
"""
import os
import sys
...
follow_running = False  # 跟随模式是否运行中
follow_thread = None
...
可用函数：forward() backward() turn_left() turn_right() stop() stand() lie_down() sit() speed_up() slow_down() nod() sway() squat() shake_head() dance() pushup() kick_left() kick_right() twist_waist() situp() bow() spread_wings() wave() march() show_off() walk_steps() get_up() swagger() left_hook() right_hook() shake_hands() lean_right() hello() follow() stop_follow()
示例：
用户：往前走两步
...
def record_audio():
    """麦克风录音，静音检测自动停止，48kHz录音后重采样到16kHz"""
    pa = pyaudio.PyAudio()
    ...
    for i in range(max_chunks):
        ...
        if energy < SILENCE_THRESHOLD:
            ...
            silence_chunks = 0
        # 至少录了2秒后才检测静音
        if i > int(2.0 * MIC_SAMPLE_RATE / MIC_CHUNK) and silence_chunks >= silence_limit:
            print("[录音] 检测到静音，停止录音")
            break
    ...
    audio_np = resample(audio_np, MIC_SAMPLE_RATE, ASR_SAMPLE_RATE)
    duration = len(audio_np) / ASR_SAMPLE_RATE
    print(f"[录音] 录音完成, 时长: {duration:.1f}s")
    return audio_np

# ==================== ASR 识别 ====================

def recognize(recognizer, audio_np):
    """用 sherpa-onnx 流式识别器识别音频"""
    stream = recognizer.create_stream()
    stream.accept_waveform(ASR_SAMPLE_RATE, audio_np.tolist())
...
def record_and_recognize(recognizer):
    ...
    for i in range(max_chunks):
        ...
        # 至少录了1.5秒后才检测静音
        if i > int(1.5 * MIC_SAMPLE_RATE / MIC_CHUNK) and silence_chunks >= silence_limit:
...
def call_llm(text):
    ...
    try:
        ...
        print("[LLM] 错误: 无法连接Ollama，请确认ollama服务已启动")
        return [], '大脑没开机，稍等一下'
    except Exception as e:
        print(f"[LLM] 错误: {e}")
        return [], '我想了想，没想明白'
...
def execute_actions(action_list):
    """执行动作列表"""
    # 收到新指令时，如果不是跟随相关的，先停掉跟随模式
    if follow_running and not any(a in ('follow()', 'stop_follow()') for a in action_list):
        stop_follow()
...
def main():
    global PuppyPosePub, PuppyGaitConfigPub, PuppyVelocityPub, RGBPub

    # 初始化ROS
    rospy.init_node('voice_interaction_local_ai')
