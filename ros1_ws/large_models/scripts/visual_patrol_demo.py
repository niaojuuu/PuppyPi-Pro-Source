...
def run(img):
    global draw_color, line_centerx, start_time, elapsed_time, enable_running
    size = (320, 240)
    ...
    if weight_sum != 0:
        ...
        if elapsed_time > 3:
            rospy.loginfo("Weight sum is zero for more than 3 seconds. Terminating program.")
            go_home_service()
            enable_running = False  
            line_centerx = -1
        else:
...
def image_callback(ros_image):
    global lock, enable_running, __isRunning, line_centerx, img_centerx
    ...
    with lock:
        if enable_running:
            ...
            if __isRunning and line_centerx != -1:
                error = line_centerx - img_centerx
                move_robot(error)
            elif __isRunning and line_centerx == -1:
...
def enter_func(msg):
    init()
    rospy.loginfo("enter visual_patrol_demo")
    rospy.sleep(0.02)
