#!/usr/bin/env python3
# coding=utf8
# large_models/scripts/puppy_control_ros1.py
...
from common import Misc, yaml_handle
...
from large_models.srv import (
    ...
    SetString, SetStringRequest, SetStringResponse
)
from puppy_control.srv import SetRunActionName, SetRunActionNameRequest, SetRunActionNameResponse
...
class PuppyController:
    def __init__(self):
        ...
        # 定义更严格的y坐标阈值
        self.y_close_lower = 300  # 增加下限，确保距离足够近
        self.y_close_upper = 310
        
        # 线程锁
        self.status_lock = threading.Lock()
        
        # 添加缺失的 self.running 属性
        self.running = True  # 这是解决 AttributeError 的关键
    ...
    def process_image(self, img):
        ...
        if self.target_color not in self.lab_data:
            rospy.logwarn(f"目标颜色 {self.target_color} 不在颜色配置中！")
            return img
        ...
        if area_max_contour is not None and area_max >= 5:
            ...
            rospy.loginfo(f"映射前的 centerX: {centerX}, centerY: {centerY}")
            ...
            rospy.loginfo(f"映射后的 centerX: {centerX}, centerY: {centerY}, radius: {radius}")
            self.block_center_point = (centerX, centerY)
            self.radius = radius

            # 打印目标位置和误差
            rospy.loginfo(f"检测到目标位置: x={self.block_center_point[0]}, y={self.block_center_point[1]}, 半径={self.radius}")
            rospy.loginfo(f"误差: x偏差={abs(self.block_center_point[0] - self.img_centerx)}")
        ...
        if self.status == PuppyStatus.STOP and self.block_color == self.target_color:
            rospy.loginfo("在 STOP 状态下检测到目标颜色，自动重新启动颜色跟踪模式")
    ...
    def is_within_clamp_range(self, y, x_error):
        ...
        """
        y_close = 295 < y <= 310  # 确保y坐标在预定范围内
        x_close = abs(x_error) < 30  # x误差在允许范围内
        rospy.loginfo(f"判断夹取范围: y_close={y_close}, x_close={x_close}")
    ...
    def adjust_for_fine_tuning(self, x_error):
        ...
        if abs(x_error) > 10:
            ...
            try:
                ...
                rospy.loginfo(f"进行微调: yaw_rate={yaw_rate}")
            except Exception as e:
                rospy.logerr(f"微调时出错: {e}")
            time.sleep(0.5)
            self.puppy.move_stop(servo_run_time=500)
            rospy.loginfo("完成微调，停止移动")
    
    def is_within_range(self):
        """判断目标是否在允许的范围内"""
        y_in_range = 295 < self.block_center_point[1] < 310
        x_in_range = abs(self.block_center_point[0] - self.img_centerx) < 30
        color_correct = self.block_color == self.target_color
        rospy.loginfo(f"判断目标是否在范围内: y_in_range={y_in_range}, x_in_range={x_in_range}, color_correct={color_correct}")
    ...
    def move(self):
        ...
        while not rospy.is_shutdown() and self.running:
            if self.status == PuppyStatus.START:
                ...
                rospy.loginfo("进入 NORMAL 状态，开始颜色跟踪")
                ...
                if self.color_tracking_enabled:
                    ...
                    rospy.loginfo(f"NORMAL 状态: y={y}, value={value}")

                    # 首先检查是否在夹取范围内
                    if self.is_within_clamp_range(y, value):
                        rospy.loginfo("满足夹取条件，进入 FOUND_TARGET 状态")
                        self.status = PuppyStatus.FOUND_TARGET
                        # 停止移动
                        try:
                            self.puppy.move_stop(servo_run_time=500)
                            rospy.loginfo("满足夹取条件，已停止移动")
                        except Exception as e:
                            rospy.logerr(f"停止移动时出错: {e}")
                    else:
                        # 初始化移动指令
                        ...
                        if y < self.y_close_lower:
                            # 目标较远，前进
                            move_x += 8.0
                            rospy.loginfo("目标较远，前进")
                        else:
                            # y >= y_close_lower，目标接近但未进入夹取范围，不再前进
                            move_x = 0.0
                            rospy.loginfo("目标接近但未进入夹取范围，停止前进")

                        # 转向控制
                        if abs(value) > 80:
                            # 目标偏离较大，快速转向
                            yaw_rate += math.radians(-8.0 * np.sign(value))
                            rospy.loginfo(f"目标偏离较大，快速转向: yaw_rate={yaw_rate}")
                        elif abs(value) > 50.0:
                            # 目标偏离中等，缓慢转向
                            yaw_rate += math.radians(-6.0 * np.sign(value))
                            rospy.loginfo(f"目标偏离中等，缓慢转向: yaw_rate={yaw_rate}")
                        ...
                        try:
                            ...
                            rospy.loginfo(f"执行移动指令: x={self.puppy_move['x']} cm, yaw_rate={self.puppy_move['yaw_rate']} radians")
                        except Exception as e:
                            rospy.logerr(f"移动指令执行时出错: {e}")
                        ...
                        if self.is_within_range():
                            ...
                            if self.y_close_lower < y < self.y_close_upper and abs(value) < 30:
                                ...
                                if self.adjusting_counter_yaw >= self.required_frames_yaw:
                                    ...
                                    self.status = PuppyStatus.ADJUSTING_YAW
                                    self.adjusting_counter_yaw = 0
                                    rospy.loginfo("进入 ADJUSTING_YAW 状态，进行方向微调")

                else:
                    try:
                        self.puppy.move_stop(servo_run_time=100)
                        rospy.loginfo("颜色跟踪未启用，停止移动")
                    except Exception as e:
                        rospy.logerr(f"停止移动时出错: {e}")
                    time.sleep(0.1)  # 逐渐停止
                ...
                try:
                    ...
                    if error_x < -10:
                        yaw_rate = math.radians(6.0)  # 向右转
                        rospy.loginfo("微调方向: 向右转")
                    elif error_x > 10:
                        yaw_rate = math.radians(-6.0)  # 向左转
                        rospy.loginfo("微调方向: 向左转")
                    ...
                    if yaw_rate != 0.0:
                        try:
                            ...
                            rospy.loginfo(f"执行方向微调：yaw_rate 调整为 {self.puppy_move['yaw_rate']} radians")
                        except Exception as e:
                            rospy.logerr(f"移动指令执行时出错: {e}")
                    ...
                    if abs(error_x) <= 10:
                        ...
                        if self.y_close_lower < y < self.y_close_upper:
                            rospy.loginfo("方向对齐完成，进入 ADJUSTING_X 状态，进行位置微调")
                            self.status = PuppyStatus.ADJUSTING_X
                        else:
                            rospy.loginfo("方向对齐，但目标物体不够近，继续前进")
                            self.status = PuppyStatus.NORMAL
                    else:
                        rospy.loginfo("方向未完全对齐，继续方向微调")
                except Exception as e:
                    rospy.logerr(f"方向微调时出错: {e}")
                    # 发生异常时，返回 NORMAL 状态以重新尝试
                    with self.status_lock:
                        self.status = PuppyStatus.NORMAL
                    rospy.loginfo("由于异常，返回 NORMAL 状态")
                ...
                try:
                    ...
                    if error_y < -10:
                        move_x = 3.0  # 向前微调
                        rospy.loginfo("微调位置: 向前移动")
                    elif error_y > 10:
                        move_x = -3.0  # 向后微调
                        rospy.loginfo("微调位置: 向后移动")
                    ...
                    if self.y_close_lower < self.block_center_point[1] < self.y_close_upper:
                        ...
                        if move_x != 0.0:
                            try:
                                ...
                                rospy.loginfo(f"执行位置微调：x 移动 {self.puppy_move['x']} cm")
                            except Exception as e:
                                rospy.logerr(f"移动指令执行时出错: {e}")
                        ...
                        # 停止移动并暂停
                        self.puppy.move_stop(servo_run_time=510)
                        rospy.loginfo("执行位置微调：停止移动")
                        time.sleep(0.51)  # 暂停0.51秒

                        # 检查是否位置对齐
                        if abs(error_y) <= 10:
                            rospy.loginfo("位置对齐完成，进入 FOUND_TARGET 状态，准备夹取")
                            self.status = PuppyStatus.FOUND_TARGET
                        else:
                            rospy.loginfo("位置未完全对齐，继续位置微调")
                    else:
                        rospy.loginfo("目标物体不够近，跳过位置微调")
                        # 如果目标不够近，返回 NORMAL 状态以继续前进
                        self.status = PuppyStatus.NORMAL
                except Exception as e:
                    rospy.logerr(f"位置微调时出错: {e}")
                    # 发生异常时，返回 NORMAL 状态以重新尝试
                    with self.status_lock:
                        self.status = PuppyStatus.NORMAL
                    rospy.loginfo("由于异常，返回 NORMAL 状态")
                ...
                try:
                    ...
                    with self.status_lock:
                        ...
                        self.color_tracking_enabled = False
                    rospy.loginfo("完成目标物块处理，进入 STOP 状态，停止颜色跟踪")
                except Exception as e:
                    rospy.logerr(f"执行 FOUND_TARGET 动作时出错: {e}")
                    # 发生异常时，返回 NORMAL 状态以重新尝试
                    with self.status_lock:
                        self.status = PuppyStatus.NORMAL
                    rospy.loginfo("由于异常，返回 NORMAL 状态")
            ...
            with self.status_lock:
                if self.status_last != self.status:
                    rospy.loginfo(f'puppyStatus 由 {self.status_last} 切换为 {self.status}')
...
class ControlServiceNode:
    def __init__(self, controller):
        ...
        rospy.loginfo('控制服务节点已启动，等待服务请求...')
    ...
    # 服务回调函数
    def enter_callback(self, request):
        """进入颜色跟踪模式的服务回调"""
        response = TriggerResponse()  # 使用 TriggerResponse 类
    ...
    def enable_callback(self, request):
        """启用或禁用颜色跟踪的服务回调"""
        response = SetBoolResponse()  # 使用 SetBoolResponse 类
        with self.controller.status_lock:
            if self.controller.status == PuppyStatus.STOP:
                response.success = False
    ...
    def set_color_callback(self, request):
        """设置目标颜色的服务回调"""
        response = SetColorResponse()  # 使用 SetColorResponse 类
        ...
        if request.color in self.controller.lab_data:
            with self.controller.status_lock:
                ...
                self.controller.color_tracking_enabled = True
                self.controller.status = PuppyStatus.START
                rospy.loginfo("颜色设置后，自动启动颜色跟踪任务")
        else:
            response.success = False
...
def main():
    # 初始化 ROS 节点
    rospy.init_node('puppy_control_ros1')
    ...
    # 启动图像显示线程
    display_thread = threading.Thread(target=display_images, args=(control_node,))
...
def display_images(control_node):
    """图像显示线程"""
    rospy.loginfo("图像显示线程已启动。")
    while not rospy.is_shutdown():
        img = control_node.get_latest_image()
        if img is not None:
            frame = img.copy()
            processed_frame = control_node.controller.process_image(frame)           
