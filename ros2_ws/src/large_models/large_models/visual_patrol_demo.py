...
class VisualPatrolDemo(Node):
    ...
    def enter_srv_callback(self, request, response):    
        self.get_logger().info('\033[1;32m%s\033[0m' % "enter visual patrol")
    ...
    def enable_running_srv_callback(self, request, response):
        ...
        self.get_logger().info('\033[1;32m%s\033[0m' % 'enable  visual_patrol_demo')
    ...
    def set_color_target_srv_callback(self, request, response):
        ...
        self.get_logger().info('\033[1;32mset color target %s\033[0m' % str(request.data))
    ...
    def move(self,):
        while True:
            self.get_logger().warn("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
            with self.lock:
                # 根据检测到的线条位置调整机器人运动
                if self.__isRunning and self.line_centerx != -1:
                    if abs(self.line_centerx - self.img_centerx) <= 50:
                        self.PuppyMove['x'] = 10.0
                        self.PuppyMove['yaw_rate'] = 0.0
                    elif self.line_centerx - self.img_centerx > 50:
                        self.PuppyMove['x'] = 8.0
                        self.PuppyMove['yaw_rate'] = -15.0
                    elif self.line_centerx - self.img_centerx < -50:
                        self.PuppyMove['x'] = 8.0
                        self.PuppyMove['yaw_rate'] = 15.0             
                    self.PuppyVelocityPub.publish(velocity_msg)
    ...
    def run(self, img):
        ...
        if weight_sum != 0:
            ...
            if self.elapsed_time > 3:           
                self.get_logger().info('Weight sum is zero for more than 3 seconds. Terminating program.')
